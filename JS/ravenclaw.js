...
const url = new URL('https://www.potterapi.com/v1/characters/'), params = {key: key, house: 'Ravenclaw' }
...
//   let character = [];
  ...
  return `<li><button id="charsBtnRaven" data-id="${character._id}" class="charsBtnRaven" value ="${character.name}">${character.name}</button></li>`
}).join('')
  document.getElementById('cha').innerHTML = chars;

  document.querySelectorAll('#charsBtnRaven').forEach(function(el){
...
fetch(url, {key: key})
 ...
 .then(function(data) {
  ...
  return `<li><button id="ravenClawBtn" data-id="${char._id}" class="charsBtnRaven" value ="${char.name}">${char.name}</button></li>`
 }).join('')
 document.getElementById('cha').innerHTML = chars;
 document.querySelectorAll('#ravenClawBtn').forEach(function(el){
  el.addEventListener('click', function(){
      // console.log(this.getAttribute('data-id')
     var charID = this.getAttribute('data-id');
     var cha = characters.filter(chat => (chat._id == charID))
