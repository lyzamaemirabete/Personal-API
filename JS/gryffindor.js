...
display.addEventListener('ready', gryffindorDisplay())
function gryffindorDisplay(){
...
//   let character = [];
  ...
  return `<li><button id="charsBtnGry" data-id="${character._id}" class="charsBtnGry" value ="${character.name}">${character.name}</button></li>`
}).join('')
  document.getElementById('cha').innerHTML = chars;

  document.querySelectorAll('#charsBtnGry').forEach(function(el){
...
fetch(url, {key: key})
 ...
 .then(function(data) {
  ...
  return `<li><button id="gryffindorBtn" data-id="${char._id}" class="charsBtnGry" value ="${char.name}">${char.name}</button></li>`
 }).join('')
 document.getElementById('cha').innerHTML = chars;
 document.querySelectorAll('#gryffindorBtn').forEach(function(el){
  el.addEventListener('click', function(){
      // console.log(this.getAttribute('data-id')
     var charID = this.getAttribute('data-id');
     var cha = characters.filter(chat => (chat._id == charID))
